...
function eventsRoutes(config) {
  ...
  router.post('/', requireRole('Owner', 'Administrator', 'Developer'), async (req, res, next) => {
    try {
      ...
      const item = await ControlEvent.create({ category, severity, title, message, resourceType, resourceId, metadata, actorId: String(req.auth.userId) });
  ...
  router.post('/:id/read', async (req, res, next) => {
    try {
      const item = await ControlEvent.findByIdAndUpdate(req.params.id, { $addToSet: { readBy: String(req.auth.userId) } }, { new: true });
