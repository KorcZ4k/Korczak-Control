...
function authRoutes(config) {
  ...
  router.post('/register', async (req, res, next) => {
    try {
      ...
      const role = (await User.countDocuments()) === 0 ? 'Owner' : 'Viewer';
  ...
  router.post('/login', async (req, res, next) => {
    try {
      ...
      const token = jwt.sign({ sub: user._id.toString(), role: user.role }, config.jwtSecret, { expiresIn: '8h', issuer: 'korczak-control-api' });
      return res.json({ token, user: safeUser(user) });
    } catch (error) { next(error); }
  });
  router.get('/me', requireAuth(config), async (req, res, next) => {
    try { const user = await User.findById(req.auth.sub); if (!user || !user.active) return res.status(401).json({ error: 'Session unavailable.' }); return res.json({ user: safeUser(user) }); }
    catch (error) { next(error); }
  });
  return router;
}
