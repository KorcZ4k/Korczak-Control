...
async function audit(user, action, details = {}) {
  ...
  await AuditLog.create({ actorAccountId: user.accountId, actorRole: user.role, targetAccountId: user.accountId, action, details });
}
...
async function migrateLegacyPermissions(user) {
  ...
  if (changed) { user.markModified('permissions'); await user.save(); }
}
...
function authRoutes(config) {
  ...
  router.post('/register', async (req, res, next) => {
    try {
      ...
      const user = await User.create({ name: name.trim(), email: normalizedEmail, passwordHash: await bcrypt.hash(password, 12), role: isFounder ? 'FOUNDER' : 'VIEWER', department: isFounder ? 'Korczak Technologies' : '', permissions: isFounder ? founderPermissions : {}, passwordChangedAt: new Date() });
      await audit(user, 'account.registered');
  ...
  router.patch('/change-password', requireAuth(config), async (req, res, next) => {
    try {
      const User = getUserModel();
      ...
      if (typeof currentPassword !== 'string' || typeof newPassword !== 'string' || newPassword.length < 12) return res.status(400).json({ error: 'A nova senha deve possuir pelo menos 12 caracteres.' });
      ...
      await user.save();
      await audit(user, 'account.password_changed');
