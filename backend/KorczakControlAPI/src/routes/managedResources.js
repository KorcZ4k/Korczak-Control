...
const getManagedResourceModel = require('../models/ManagedResource');
const { requireAuth, requireRole } = require('../middleware/auth');

function defaultsFor(kind, config) {
  if (kind === 'api') return [
    { name: 'Korczak Control', slug: 'korczak-control', kind, url: config.kzControlApi || '', technology: 'Node.js · Express', status: config.kzControlApi ? 'operational' : 'unknown', notes: 'API administrativa principal.' },
    { name: 'Korczak Tech Site', slug: 'korczak-tech-site', kind, url: config.kzSiteApi || '', technology: 'HTTP API', status: config.kzSiteApi ? 'operational' : 'unknown', notes: 'Serviço do ecossistema Korczak Technologies.' }
  ];
  return [
    { name: 'Korczak Control', slug: 'korczak-control', kind, technology: 'Android · Kotlin · Jetpack Compose', status: 'operational', version: config.version, notes: 'Aplicativo administrativo oficial.' }
  ];
}
...
function managedResourcesRoutes(config) {
  ...
  const resources = () => getManagedResourceModel();
  ...
  router.get('/:kind', async (req, res, next) => {
    try {
      ...
      const items = await resources().find({ kind }).sort({ name: 1 }).lean();
  ...
  router.get('/:kind/:slug', async (req, res, next) => {
    try {
      ...
      if (!['api', 'app'].includes(kind)) return res.status(404).json({ error: 'Resource type not found.', requestId: req.requestId });
      const item = await resources().findOne({ kind, slug: req.params.slug }).lean();
  ...
  router.post('/:kind', requireRole('Owner', 'Administrator', 'Developer'), async (req, res, next) => {
    try {
      ...
      const item = await resources().create({ name, slug, kind, url, repository, technology, version, status, latencyMs, notes, lastActivityAt: new Date() });
  ...
  router.patch('/:kind/:slug', requireRole('Owner', 'Administrator', 'Developer'), async (req, res, next) => {
    try {
      ...
      const item = await resources().findOneAndUpdate({ kind: req.params.kind, slug: req.params.slug }, { $set: update }, { new: true, runValidators: true });
