...
const { databaseExtrasRoutes } = require('./routes/databaseExtras');
...
const { controlCenterRoutes } = require('./routes/controlCenter');
const config = loadConfig(); const app = express(); const startedAt = Date.now();
app.disable('x-powered-by'); app.use(helmet({ contentSecurityPolicy: false })); app.use(express.json({ limit: '256kb' })); app.use((req, res, next) => { req.requestId = crypto.randomUUID(); res.setHeader('X-Request-Id', req.requestId); next(); });
const allowedOrigins = config.corsOrigin.split(',').map((v) => v.trim()).filter(Boolean); app.use(cors({ origin(origin, cb) { if (!origin || allowedOrigins.length === 0 || allowedOrigins.includes(origin)) return cb(null, true); return cb(new Error('Origin not allowed by CORS.')); }, methods: ['GET', 'POST', 'PATCH', 'DELETE'], allowedHeaders: ['Content-Type', 'Authorization', 'x-bootstrap-token'] }));
...
function shutdown(signal) { console.log(`${signal} received. Closing server.`); if (!server) return process.exit(0); server.close((error) => process.exit(error ? 1 : 0)); } process.on('SIGTERM', () => shutdown('SIGTERM')); process.on('SIGINT', () => shutdown('SIGINT')); start().catch((error) => { console.error('Failed to start API:', error); process.exit(1); });
