...
fun ControlApp(onLogout: () -> Unit) {
    ...
    val bottomDestinations = listOf("dashboard", "integrations", "organization", "notifications", "settings").mapNotNull { route -> visibleDestinations.firstOrNull { it.route == route } }
    ...
    if (update != null) {
        AlertDialog(
            ...
            dismissButton = {
                if (updateProgress == null) TextButton(onClick = { update = null }) { Text("Agora não") }
            }
        )
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Column { Text("KORCZAK CONTROL"); Text("${session.accountName()} · ${currentDestination.label}", style = MaterialTheme.typography.labelMedium) } },
                actions = {
                    ...
                    DropdownMenu(expanded = showModules, onDismissRequest = { showModules = false }) {
                        visibleDestinations.filter { destination -> bottomDestinations.none { it.route == destination.route } }.forEach { item ->
                            DropdownMenuItem(
                                ...
                                onClick = { showModules = false; navController.navigate(item.route) { launchSingleTop = true; restoreState = true } }
                            )
                        }
                    }
        ...
        NavHost(navController, "dashboard", Modifier.fillMaxSize().padding(padding)) {
            ...
            visibleDestinations.filter { it.route !in listOf("dashboard", "integrations", "organization", "settings", "notifications") }.forEach { item ->
                composable(item.route) { OperationsScreen(item.route) }
            }
            composable("notifications") { OperationsScreen("profile") }
        }
    }
}
